...
//  AliAODInputHandler* handler = new AliAODInputHandler();
  ...
  gROOT->LoadMacro("AliAnalysisTaskUpcTree.cxx+g");
  gROOT->LoadMacro("AddTaskUpcTree.C"); // load the addtask macro
  AliAnalysisTaskUpcTree *ana = AddTaskUpcTree();
  mgr->AddTask(ana);
  ...
  if(local)
  {
    ...
    chain->Add("~/NBI/triggerHMstudies/hmtfTrigTask/alice/data/2016/LHC16k/000258537/pass1/16000258537020.603/AliESDs.root");
